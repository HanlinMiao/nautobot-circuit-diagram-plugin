...
namespace = Collection("nautobot_circuit_diagram_plugin")
namespace.configure(
    {
        "nautobot_circuit_diagram_plugin": {
            "nautobot_ver": "1.1.0",
            "project_name": "nautobot_circuit_diagram_plugin",
            "python_ver": "3.7",
            "local": False,
...
def docker_compose(context, command, **kwargs):
    ...
    """
    build_env = {
        "NAUTOBOT_VER": context.nautobot_circuit_diagram_plugin.nautobot_ver,
        "PYTHON_VER": context.nautobot_circuit_diagram_plugin.python_ver,
    }
    compose_command = f'docker-compose --project-name {context.nautobot_circuit_diagram_plugin.project_name} --project-directory "{context.nautobot_circuit_diagram_plugin.compose_dir}"'
    for compose_file in context.nautobot_circuit_diagram_plugin.compose_files:
        compose_file_path = os.path.join(context.nautobot_circuit_diagram_plugin.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.nautobot_circuit_diagram_plugin.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_circuit_diagram_plugin.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    """
    command = "nautobot-server makemigrations nautobot_circuit_diagram_plugin"
...
# ------------------------------------------------------------------------------
@task
def unittest(context, label="nautobot_circuit_diagram_plugin"):
...
def pylint(context):
    ...
    Args:
        context (obj): Used to run specific commands
    """
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml nautobot_circuit_diagram_plugin'
    run_command(context, command)
...
def tests(context):
    ...
    Args:
        context (obj): Used to run specific commands
    """
    if not is_truthy(context.nautobot_circuit_diagram_plugin.local):
